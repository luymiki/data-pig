...
/**
 * Configuration 提供多级JSON配置信息无损存储<br><br>
 * 实例代码:<br><br>
 *    获取job的配置信息:<br>
 *    		Configuration configuration = Configuration.from(new File("CodeGeneratorConfig.json"));<br>
 *    		String jobContainerClass = configuration.getString("core.container.job.class");<br><br>
 * 	  设置多级List:<br>
 * 			configuration.set("job.reader.parameter.jdbcUrl", Arrays.asList(new String[] {"jdbc", "jdbc"}));<br><br>
 *    合并Configuration:<br>
 *    		configuration.merge(another);<br><br>
 * Configuration 存在两种较好地实现方式<br>
 * 第一种是将JSON配置信息中所有的Key全部打平，用a.b.c的级联方式作为Map的Key，内部使用一个Map保存信息<br>
 * 第二种是将JSON的对象直接使用结构化树形结构保存<br><br>
 * 目前使用的第二种实现方式，使用第一种的问题在于:<br>
 * 1.插入新对象，比较难处理，例如a.b.c="bazhen"，此时如果需要插入a="bazhen"，也即是根目录下第一层所有类型全部要废弃 ，使用"bazhen"作为value，第一种方式使用字符串表示key，难以处理这类问题。<br>
...
public class Configuration {
    ...
    public static Configuration from(String json) {
        ...
        try {
            ...
            throw new RuntimeException("从JSON字符串加载Configuration异常", e);
        }
    ...
    public static Configuration from(File file) {
        try {
            return Configuration.from(IOUtils.toString(new FileInputStream(file), Charset.forName("utf-8")));
        } catch (FileNotFoundException e) {
            throw new RuntimeException(
                    String.format("配置信息错误，您提供的配置文件[%s]不存在. 请检查您的配置文件.", file.getAbsolutePath()));
        } catch (IOException e) {
            throw new RuntimeException(String.format("配置信息错误. 您提供配置文件[%s]读取失败，错误原因: %s. 请检查您的配置文件的权限设置.",
    ...
    public static Configuration from(InputStream is) {
        try {
            return Configuration.from(IOUtils.toString(is, Charset.forName("utf-8")));
        } catch (IOException e) {
            throw new RuntimeException(
                    String.format("请检查您的配置文件. 您提供的配置文件读取失败，错误原因: %s. 请检查您的配置文件的权限设置.", e.getMessage()), e);
        }
    }
    ...
    public Character getChar(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 因为配置文件路径[%s] 值非法，期望是字符类型. 请检查您的配置并作出修改.", path), e);
        }
    }
    ...
    public Long getLong(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是整数类型. 请检查您的配置并作出修改.", path), e);
        }
    }
    ...
    public Double getDouble(final String path) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("任务读取配置文件出错. 配置文件路径[%s] 值非法, 期望是浮点类型. 请检查您的配置并作出修改.", path), e);
        }
    }
...
//		this.checkPath(path);
     ...
     */
    public Object setAndReturnOldValue(final String path, final Object object) {
        checkPath(path);
...
//		Set<String> collect = new HashSet<String>();
    ...
    public Object remove(final String path) {
        ...
        if (null == result) {
            throw new RuntimeException(String.format("配置文件对应Key[%s]并不存在，该情况是代码编程错误.", path));
        }
        //---注释掉this.set(path, null)，从置空改为移除 start
        this.removeObjectRecursive(this.root, split2List(path), 0);
        //---注释掉this.set(path, null)，从置空改为移除 end
...
//		this.set(path, null);
    ...
    /**
     * 将Configuration作为JSON格式化输出
     */
    public String toJSONPrettyFormat() {
        return JSON.toJSONString(this.getInternal(), SerializerFeature.PrettyFormat, SerializerFeature.WriteMapNullValue);
    }
    ...
    void getKeysRecursive(final Object current, String path, Set<String> collect) {
        ...
        if (isMap) {
            ...
            for (final String key : mapping.keySet()) {
                if (StrUtil.hasWhitespace(key)) {
                    LOG.warn(String.format("警告：路径[%s]存在空白字符！", key));
                }
                if (StringUtils.isBlank(path)) {
                    getKeysRecursive(mapping.get(key), key/*key.trim()*/, collect);
                } else {
                    getKeysRecursive(mapping.get(key), path + "." + key/*key.trim()*/,
                            collect);
                }
            }
            return;
    ...
    private void setObject(final String path, final Object object) {
        ...
        throw new RuntimeException(String.format("值[%s]无法适配您提供[%s]， 该异常代表系统编程错误.",
    ...
    Object buildObject(final List<String> paths, final Object object) {
        if (null == paths) {
            throw new RuntimeException("Path不能为null，该异常代表系统编程错误.");
        }
...
//		String path = paths.get(index).trim();
        ...
        if (StrUtil.hasWhitespace(path)) {
            LOG.warn(String.format("警告：路径[%s]存在空白字符！", path));
        }
        ...
        throw new RuntimeException("该异常代表系统编程错误.");
    }

    void removeObjectRecursive(Object current, final List<String> paths, int index) {
        String path = paths.get(index);
        if (StrUtil.hasWhitespace(path)) {
            LOG.warn(String.format("警告：路径[%s]存在空白字符！", path));
        }
        ...
        if (isNeedMap) {
            ...
            if (index == paths.size() - 1) {//已经是最后一位
                mapping.remove(path);
                return;
            }
            current = mapping.get(path);
            removeObjectRecursive(current, paths, index + 1);
            return;
        }
    ...
    private Object findObjectInMap(final Object target, final String index) {
        ...
        if (null == result) {
            throw new IllegalArgumentException(String.format(
                    "您提供的配置文件有误. 路径[%s]值为null，系统无法识别该配置. 请检查您的配置并作出修改.", index));
        }
    ...
    private Object findObjectInList(final Object target, final String each) {
        ...
        if (!StringUtils.isNumeric(index)) {
            throw new IllegalArgumentException(
                    String.format("系统编程错误，列表下标必须为数字类型，但该节点发现实际类型是[%s] ，该异常代表系统编程错误.", index));
        }
    ...
    private void checkPath(final String path) {
        if (null == path) {
            throw new IllegalArgumentException("系统编程错误, 该异常代表系统编程错误.");
        }
