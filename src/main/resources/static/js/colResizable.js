...
(function($) {
    ...
    function onDragingStart(e){
        ...
        if(resizable){
            ...
            //创建参考线
            if(!leftLine){
                leftLine = $("<div style=' width:0px;border-style: dashed;border-color: #1528ff;border-width: 0 1px 0 0px;position:absolute; '></div>");
                leftLine.appendTo("body");
                rightLine = leftLine.clone();
                rightLine.appendTo("body");
            }
            //显示参考线
            leftLine.css({"top":resizeHeader.offset().top-40, "left": resizeHeader.offset().left, "height": 200});
            rightLine.css({"top":resizeHeader.offset().top-40, "left": e.pageX, "css":"col-resize", "height": 200});
            leftLine.show();
            rightLine.show();
            //设置为已经在拖动
            resizing = true;
    ...
    function doResize(){
        ...
        var w = dataTable.width() + (newWidth-owidth);
        dataTable.width(w);
...
//         //设置新列宽
//         resizeHeader.width(newWidth);
//         var w = headerTable.width() + (newWidth-owidth);
//         headerTable.width(w);
//
//         var dths = dataTable.find("thead tr th");
//         var hths = headerTable.find("thead tr th");
//         for(var i=0; i<hths.length;i++){
//             $(hths[i]).find(".th-inner").width($(hths[i]).width()-14);
//             $(dths[i]).width($(hths[i]).width());
//         }
//         headerTable.width(w);
//         dataTable.width(w);
//     }
